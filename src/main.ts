...
thead?.addEventListener('click', (e)=>{
  const target = (e.target as HTMLElement).closest('th') as HTMLTableCellElement
  // console.log(target?.dataset.name)
  if (target.textContent == target?.dataset.name) {
    if (th) th.textContent = th.dataset.name as string
...
// В современной разработке стадия погружения используется очень редко, обычно события обрабатываются во время всплытия
...
function runOnKeys(fn:Function, ...codes: string[]) {
...
document.addEventListener('keydown',(e)=>{
  ...
  runOnKeys(() => { alert('boom') }, 'KeyQ', 'KeyW', 'KeyE')
})
...
// !Замыкания
// Забыл записать видео. Читать тут https://learn.javascript.ru/closure
